---
title: "React Paris Meetup #13 @ Neosoft - 24/03/2026"
---

layout: mainSection

---

import { Side, Text, Image } from "@gpichot/spectacle-deck";
import { ItemsColumn } from "@gpichot/spectacle-deck";
import { talks, communityMeetups, orgs, eventDetails } from "./data";
import { TalkCard, TalkList } from "../common/TalkCard";
import { OrgCard, OrgList } from "../common/OrgCard";
import { EventCard, EventList } from "../common/EventCard";

# React Paris Meetup #13

---

layout: centered

---

## **[Code of Conduct](https://github.com/ReactParis/.github/blob/main/CODE_OF_CONDUCT.markdown)**

https://link.infini.fr/code-of-conduct

:qrcode[https://github.com/ReactParis]

Be respectful

---

layout: centered

---

## Team

<OrgList orgs={orgs} />

---

layout: sidedImage

---

## **Talks**

import weNeedYou from "../common/minion-we-need-you.jpeg";

<Image
  src={weNeedYou}
  alt="We need you"
  style={{ maxHeight: "50vh", objectFit: "contain" }}
/>

Want to speak at our next meetup?

- React 19 & React Compiler (new features, deep dive)
- AI integration (best practices, ROI, future of React + AI)
- Performance (build optimization, data loading, frontend perf)
- Modern tooling (Next.js, Turborepo, Tanstack Start, CSS management)
- Architecture & patterns (scalable code, senior-level development)

---

layout: side

---

## **Where to contact us**

<Side>

- [GitHub](https://github.com/ReactParis)

- [Discord](https://discord.gg/EGeZmfJbKB)

<div style={{alignSelf: 'center'}}>

:qrcode[https://discord.gg/EGeZmfJbKB]

</div>

- LinkedIn, Twitter, Meetup, etc.

</Side>

---

layout: side

---

## **Community Events**

<Side>
<EventList>

{communityMeetups.map((meetup) => (

<EventCard key={meetup.title} event={meetup} />

))}

</EventList>
</Side>

---

layout: side

---

## **Tonight's Talks**

<Side>
  <TalkList>

{talks.map((talk) => (

<TalkCard key={talk.title} talk={talk} />

))}

  </TalkList>
</Side>

---

layout: centered

---

<Image
  src={neosoft}
  width="30vw"
  height="30vh"
  style={{ objectFit: "contain", backgroundColor: "transparent" }}
/>

<br />
<br />

### **Tonight's host: {eventDetails.host}**
